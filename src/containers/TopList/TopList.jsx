import React,{Component} from 'react';
import './css/TopList.less';

export default class TopList extends Component{

  render(){
    return(
      <div className="kjcTopListContainer kjcW">
        <div className="kjcLeftNav">
          <div className="kjcFeatureList">
            <h2>云音乐特色榜</h2>
            <ul>
              <li className="active">
                <img src="http://p2.music.126.net/DrRIg6CrgDfVLEph9SNh7w==/18696095720518497.jpg?param=40y40" alt=""/>
                <div className="kjcFeatureItem">
                  <div className="kjcFeatureTitle">云音乐飙升榜</div>
                  <div className="kjcFeatureUpdate">每天更新</div>
                </div>
              </li>
              <li>
                <img src="http://p2.music.126.net/N2HO5xfYEqyQ8q6oxCw8IQ==/18713687906568048.jpg?param=40y40" alt=""/>
                <div className="kjcFeatureItem">
                  <div className="kjcFeatureTitle">云音乐新歌榜</div>
                  <div className="kjcFeatureUpdate">每天更新</div>
                </div>
              </li>
              <li>
                <img src="http://p2.music.126.net/sBzD11nforcuh1jdLSgX7g==/18740076185638788.jpg?param=40y40" alt=""/>
                <div className="kjcFeatureItem">
                  <div className="kjcFeatureTitle">网易原创歌曲榜</div>
                  <div className="kjcFeatureUpdate">每周四更新</div>
                </div>
              </li>
              <li>
                <img src="http://p2.music.126.net/GhhuF6Ep5Tq9IEvLsyCN7w==/18708190348409091.jpg?param=40y40" alt=""/>
                <div className="kjcFeatureItem">
                  <div className="kjcFeatureTitle">云音乐热歌榜</div>
                  <div className="kjcFeatureUpdate">每周四更新</div>
                </div>
              </li>
            </ul>
          </div>
          <div className="kjcGlobalList">
            <h2>全球媒体榜</h2>
            <ul>
              <li>
                <img src="http://p2.music.126.net/y8zyTt4HwXbJqB31aQKz1A==/109951164353220919.jpg?param=40y40" alt=""/>
                <div className="kjcGlobalItem">
                  <div className="kjcGlobalTitle">云音乐说唱榜</div>
                  <div className="kjcGlobalUpdate">每天更新</div>
                </div>
              </li>
              <li>
                <img src="http://p2.music.126.net/BzSxoj6O1LQPlFceDn-LKw==/18681802069355169.jpg?param=40y40" alt=""/>
                <div className="kjcGlobalItem">
                  <div className="kjcGlobalTitle">云音乐古典音乐榜</div>
                  <div className="kjcGlobalUpdate">每天更新</div>
                </div>
              </li>
              <li>
                <img src="http://p2.music.126.net/5tgOCD4jiPKBGt7znJl-2g==/18822539557941307.jpg?param=40y40" alt=""/>
                <div className="kjcGlobalItem">
                  <div className="kjcGlobalTitle">云音乐电音榜</div>
                  <div className="kjcGlobalUpdate">每天更新</div>
                </div>
              </li>
              <li>
                <img src="http://p2.music.126.net/oUxnXXvM33OUHxxukYnUjQ==/109951164174523461.jpg?param=40y40" alt=""/>
                <div className="kjcGlobalItem">
                  <div className="kjcGlobalTitle">抖音排行榜</div>
                  <div className="kjcGlobalUpdate">每天更新</div>
                </div>
              </li>
              <li>
                <img src="http://p2.music.126.net/XbjRDARP1xv5a-40ZDOy6A==/109951163785427934.jpg?param=40y40" alt=""/>
                <div className="kjcGlobalItem">
                  <div className="kjcGlobalTitle">新声榜</div>
                  <div className="kjcGlobalUpdate">每周一更新</div>
                </div>
              </li>
              <li>
                <img src="http://p2.music.126.net/vttjtRjL75Q4DEnjRsO8-A==/18752170813539664.jpg?param=40y40" alt=""/>
                <div className="kjcGlobalItem">
                  <div className="kjcGlobalTitle">云音乐ACG音乐榜</div>
                  <div className="kjcGlobalUpdate">每周一更新</div>
                </div>
              </li>
              <li>
                <img src="http://p2.music.126.net/vs-cMh49e6qPEorHuhU07A==/18737877162497499.jpg?param=40y40" alt=""/>
                <div className="kjcGlobalItem">
                  <div className="kjcGlobalTitle">云音乐韩语榜</div>
                  <div className="kjcGlobalUpdate">每周一更新</div>
                </div>
              </li>
              <li>
                <img src="http://p2.music.126.net/vs-cMh49e6qPEorHuhU07A==/18737877162497499.jpg?param=40y40" alt=""/>
                <div className="kjcGlobalItem">
                  <div className="kjcGlobalTitle">云音乐韩语榜</div>
                  <div className="kjcGlobalUpdate">每周一更新</div>
                </div>
              </li>
              <li>
                <img src="http://p2.music.126.net/vs-cMh49e6qPEorHuhU07A==/18737877162497499.jpg?param=40y40" alt=""/>
                <div className="kjcGlobalItem">
                  <div className="kjcGlobalTitle">云音乐韩语榜</div>
                  <div className="kjcGlobalUpdate">每周一更新</div>
                </div>
              </li>
              <li>
                <img src="http://p2.music.126.net/vs-cMh49e6qPEorHuhU07A==/18737877162497499.jpg?param=40y40" alt=""/>
                <div className="kjcGlobalItem">
                  <div className="kjcGlobalTitle">云音乐韩语榜</div>
                  <div className="kjcGlobalUpdate">每周一更新</div>
                </div>
              </li>
              <li>
                <img src="http://p2.music.126.net/vs-cMh49e6qPEorHuhU07A==/18737877162497499.jpg?param=40y40" alt=""/>
                <div className="kjcGlobalItem">
                  <div className="kjcGlobalTitle">云音乐韩语榜</div>
                  <div className="kjcGlobalUpdate">每周一更新</div>
                </div>
              </li>
              <li>
                <img src="http://p2.music.126.net/vs-cMh49e6qPEorHuhU07A==/18737877162497499.jpg?param=40y40" alt=""/>
                <div className="kjcGlobalItem">
                  <div className="kjcGlobalTitle">云音乐韩语榜</div>
                  <div className="kjcGlobalUpdate">每周一更新</div>
                </div>
              </li>
              <li>
                <img src="http://p2.music.126.net/vs-cMh49e6qPEorHuhU07A==/18737877162497499.jpg?param=40y40" alt=""/>
                <div className="kjcGlobalItem">
                  <div className="kjcGlobalTitle">云音乐韩语榜</div>
                  <div className="kjcGlobalUpdate">每周一更新</div>
                </div>
              </li>
              <li>
                <img src="http://p2.music.126.net/vs-cMh49e6qPEorHuhU07A==/18737877162497499.jpg?param=40y40" alt=""/>
                <div className="kjcGlobalItem">
                  <div className="kjcGlobalTitle">云音乐韩语榜</div>
                  <div className="kjcGlobalUpdate">每周一更新</div>
                </div>
              </li>
              <li>
                <img src="http://p2.music.126.net/vs-cMh49e6qPEorHuhU07A==/18737877162497499.jpg?param=40y40" alt=""/>
                <div className="kjcGlobalItem">
                  <div className="kjcGlobalTitle">云音乐韩语榜</div>
                  <div className="kjcGlobalUpdate">每周一更新</div>
                </div>
              </li>
              <li>
                <img src="http://p2.music.126.net/vs-cMh49e6qPEorHuhU07A==/18737877162497499.jpg?param=40y40" alt=""/>
                <div className="kjcGlobalItem">
                  <div className="kjcGlobalTitle">云音乐韩语榜</div>
                  <div className="kjcGlobalUpdate">每周一更新</div>
                </div>
              </li>
            </ul>
          </div>
        </div>
        <div className="kjcRightContent">
          <div className="kjcContentHeader">
            <div className="kjcImgWrap">
             <img src="http://p2.music.126.net/DrRIg6CrgDfVLEph9SNh7w==/18696095720518497.jpg?param=150y150" alt=""/>
            </div>
            <div className="kjcContentMain">
              <h2>云音乐飙升榜</h2>
              <div className="kjcContentUser">
                <i className="kjcIcon"></i>
                <span className="kjcUpdateTime">最近更新：01月06日</span>
                <span className="kjcUpdateCount">（每天更新）</span>
              </div>
              <div className="kjcBtns">
                <div className="kjcPlay">
                  <span>
                    <em className="ply"></em>
                    播放
                  </span>
                </div>
                <div className="kjcAdd"></div>
                <div className="kjcCollection"><span>(2660728)</span></div>
                <div className="kjcShare"><span>(7540)</span></div>
                <div className="kjcDownload"><span>下载</span></div>
                <div className="kjcComment"><span>(165465)</span></div>
              </div>
            </div>
          </div>
          <div className="kjcSongList">
            <div className="kjcTitleWrap">
              <h2 className="kjcTitleName">歌曲列表</h2>
              <span className="kjcOne">100首歌</span>
              <span className="kjcPlayCount">
                播放：
                <span className="kjcCount">2725255424</span>
                次
              </span>
            </div>
            <div className="kjcListContent">
              <div className="kjcTableHeader">
                <div className="kjcTemp"><div></div></div>
                <div className="kjcNameTitle"><div>标题</div></div>
                <div className="kjcHowLong"><div>时长</div></div>
                <div className="kjcSinger"><div>歌手</div></div>
              </div>
              <ul className="kjcSongListContent">
                <li className="kjcSongItem">
                  <div className="kjcNumAndRank">
                    <div className="kjcSongNum">1</div>
                    <div className="kjcSongRank">7</div>
                  </div>
                  <div className="kjcSongContent">
                    <img src="http://p1.music.126.net/zgDvTPDSYKjQUjkV8sQUOA==/109951164605092552.jpg?param=50y50&quality=100" alt=""/>
                    <span className="kjcOpen">&nbsp;</span>
                    <div className="kjcSingerContent">
                      大眠 (完整版)&nbsp;
                    </div>
                    <span className="kjcOriginSinger"> - (原唱：王心凌)</span>
                  </div>
                  <div className="kjcSongTime">
                    <span>04:31</span>
                  </div>
                  <div className="kjcSinger">
                    <span>
                    小乐哥
                    </span>
                  </div>
                </li>
              <li className="kjcSongItem">
                  <div className="kjcNumAndRank">
                    <div className="kjcSongNum">2</div>
                    <div className="kjcSongRank">2</div>
                  </div>
                  <div className="kjcSongContent">
                    <img src="http://p1.music.126.net/hWHt1pPaf3mxkO-vLQbhKg==/109951164400130453.jpg?param=50y50&quality=100" alt=""/>
                    <span className="kjcOpen">&nbsp;</span>
                    <div className="kjcSingerContent">
                      太阳&nbsp;
                    </div>
                  </div>
                  <div className="kjcSongTime">
                    <span>03:04</span>
                  </div>
                  <div className="kjcSinger">
                    <span>
                    王巨星
                    </span>
                  </div>
                </li>
              <li className="kjcSongItem">
                  <div className="kjcNumAndRank">
                    <div className="kjcSongNum">3</div>
                    <div className="kjcSongRank new"></div>
                  </div>
                  <div className="kjcSongContent">
                    <img src="http://p1.music.126.net/2lC8dgQola_6YGfeJJZdtA==/109951164601966699.jpg?param=50y50&quality=100" alt=""/>
                    <span className="kjcOpen">&nbsp;</span>
                    <div className="kjcSingerContent">
                      赤脚追光&nbsp;
                    </div>
                    <div className="kjcMV"></div>
                  </div>
                  <div className="kjcSongTime">
                    <span>03:51</span>
                  </div>
                  <div className="kjcSinger">
                    <span>
                    R1SE
                    </span>
                  </div>
                </li>
              <li className="kjcSongItem kjcAfter">
                  <div className="kjcNumAndRank">
                    <div className="kjcSongNum">4</div>
                    <div className="kjcSongRank new"></div>
                  </div>
                  <div className="kjcSongContent">
                    <span className="kjcOpen">&nbsp;</span>
                    <div className="kjcSingerContent">
                      勇气&nbsp;
                    </div>
                  </div>
                  <div className="kjcSongTime">
                    <span>04:01</span>
                  </div>
                  <div className="kjcSinger">
                    <span>
                    棉子
                    </span>
                  </div>
                </li>
              <li className="kjcSongItem kjcAfter">
                <div className="kjcNumAndRank">
                  <div className="kjcSongNum">4</div>
                  <div className="kjcSongRank new"></div>
                </div>
                <div className="kjcSongContent">
                  <span className="kjcOpen">&nbsp;</span>
                  <div className="kjcSingerContent">
                    勇气&nbsp;
                  </div>
                </div>
                <div className="kjcSongTime">
                  <span>04:01</span>
                </div>
                <div className="kjcSinger">
                  <span>
                  棉子
                  </span>
                </div>
              </li>
              <li className="kjcSongItem kjcAfter">
                  <div className="kjcNumAndRank">
                    <div className="kjcSongNum">4</div>
                    <div className="kjcSongRank new"></div>
                  </div>
                  <div className="kjcSongContent">
                    <span className="kjcOpen">&nbsp;</span>
                    <div className="kjcSingerContent">
                      勇气&nbsp;
                    </div>
                  </div>
                  <div className="kjcSongTime">
                    <span>04:01</span>
                  </div>
                  <div className="kjcSinger">
                    <span>
                    棉子
                    </span>
                  </div>
                </li>
              </ul>
            </div>
          </div>
          <div className="kjcCommendContainer">
              <div className="kjcCommendWrap">
                <h2 className="kjcCommendName">评论</h2>
                <span className="kjcCommendCount">共165468条评论</span>
              </div>
              <div class="kjcCommendContent">
                <div class="kjcWriteCommend">
                  <img src="http://s4.music.126.net/style/web2/img/default/default_avatar.jpg?param=50y50" alt=""/>
                  <div class="kjcCommendBox">
                    <textarea class="kjcUserContent" placeholder="评论"></textarea>
                    <i class="kjcArrow"></i>
                    <div class="kjcUserBtns">
                      <i class="kjcAt"></i>
                      <i class="kjcExp"></i>
                      <span class="kjcWord">140</span>
                      <div class="kjcCommendBtn">评论</div>
                    </div>
                  </div>
                </div>
                <div class="kjcStarCommend">
                  <div class="kjcStarTitle">精彩评论</div>
                  <ul class="kjcStarList">
                    <li class="kjcStarItem">
                      <img src="https://p1.music.126.net/zoP7jqd1soAXDa1h0xCNLA==/109951164532777256.jpg?param=50y50" alt=""/>
                      <div class="kjcStarItemContent">
                          <div class="kjcStarTop">
                            <span class="kjcUserName">发发火星一族 : </span>
                            <span class="kjcUserContent">我们的《好想爱这个世界啊》还在飚</span>
                          </div>
                          <div class="kjcStarBtns">
                            <span class="kjcStarTime">11:02</span>
                            <span class="kjcStarGood">
                              <i></i>
                              <span class="kjcStarCount">(11)</span>
                            </span>
                            <i>|</i>
                            <span class="kjcResponse">回复</span>
                          </div>
                      </div>
                    </li>
                  </ul>
                </div>
              </div>
         
          </div>
        </div>
      </div>
    )
  }
}